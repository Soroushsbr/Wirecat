...
- Implemented with Android's `VpnService`, using `addAllowedApplication()` so *only* the apps you pick are affected.
- No root required.
...
### Session history
- Every monitoring session is saved locally (SQLite via a plain `SQLiteOpenHelper`) with its full per-app breakdown, browsable later from the History screen.
...
This project intentionally does **not** ship a Gradle wrapper.
